...
昆明湖 V2R2-930 MMU（Memory Management Unit） 支持 RISC-V SV39/SV48 分页机制。其作用主要有：

- 地址转换：将虚拟地址（39 位/48 位）转换为物理地址（48 位）；
...
1. Page Cache：缓存页表，Sv39/Sv48 的 3 层页表全部且分开缓存，可以一拍完成 3 层信息的查询；
...
![Sv39x4 两阶段地址翻译](figs/2steptranslate.svg)
