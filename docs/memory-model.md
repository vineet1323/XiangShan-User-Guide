...
昆明湖 V2R2-930 MMU（Memory Management Unit） 支持 RISC-V SV39 分页机制。其作用主要有：

- 地址转换：将虚拟地址（39 位）转换为物理地址（36 位）；
...
昆明湖 V2R2-930 MMU 采用两级 TLB ，第一级为 L1 TLB ，分别为指令 ITLB 与数据 DTLB ，第二级为 L2 TLB 
...
取值请求、加载与存储请求命中 ITLB 与 DTLB 时，当拍就可以得到物理地址与对应权限属性

L2 TLB 为指令和数据共用，L2 TLB 包含五个主要单元：
...
4. Miss Queue：缓存查询 Page Cache 和 Last Level Page Walker 的 miss 请求；
5. Prefetcher：预取器。

#### 地址转换流程

MMU 负责将虚拟地址翻译成物理地址，并用翻译得到的物理地址进行访存。昆明湖 V2R2-930 支持 Sv39 分页机制，虚拟地址长度为 39 位，低 12 位是页内偏移，高 27 位分为三段（每段 9 位），也就是三级页表。
...
- PMA 模式兼容原来未定义虚拟地址 PMA 时的系统属性设定，遵循原物理地址 PMA 属性，即 sysmap属性；
...
| [59:44] | ASID | 地址空间标识符。ASID 的长度可参数化配置，对于香山昆明湖架构采用的 Sv39地址转换模式，ASID 长度最大值为16 |
...
| **位** | **域** | **描述** |
| --- | --- | --- |
| 7 | L | 表示该PMP表项是否被锁定。该域默认为 0，表示未被锁定；为1时表示被锁定 |
| [6:5] | 保留 | 保留位，默认为 0 |
...
PMA 的实现采用类似 PMP 的方式，PMA 寄存器默认拥有初始值， 需手动设置与平台地址属性一致。PMA 寄存器利用了 M 态 CSR 的保留寄存器地址空间，默认为 16 项，可参数化修改。
...
| 1 | W | 表示该 PMA 表项配置的地址是否支持写。该域为 1 时表示支持写，为 0时不支持写 |
| 0 | R | 表示该 PMA 表项配置的地址是否支持读。该域为 1 时表示支持读，为 0时不支持读 |
...
昆明湖 V2R2-930 MMU 模块可能产生 4 中异常，包括 guest page fault、page fault、access fault以及 L2 TLB page cache 的 ECC 校验出错。根据请求来源分别交付给对应模块处理。
...
另外，根据 RISC-V 手册，Page Fault 的优先级高于 Access Fault，但是如果 Page Table Walker 或 Last Level Page Table Walker 在 PMP 检查或 PMA 检查时出现 Access Fault，此时页表项为非法，会发生 Page Fault 和 Access Fault 一起出现的特殊情况，昆明湖 V2R2-930 选择报 Access Fault。其余情况下均满足Page Fault的优先级高于Access Fault。
